...
import com.akicat.knowledgeshare.repository.NoteUploadRepository;
...
public class FileUploadServiceImp implements FileUploadService {
    ...
    @Autowired
    private NoteUploadRepository noteUploadRepository;
    ...
    @Override
    public int uploadNote(NoteEntity note) {
        if(noteUploadRepository.uploadNote(note) != 1){
    ...
    public int updateNote(NoteEntity note) {
        if(noteUploadRepository.updateNote(note) != 1){
            throw new BusinessFailureException(true);
        }
        else
            return 1;
